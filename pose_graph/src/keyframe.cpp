                   ...
                   vector<cv::Point2f>& _point_2d_norm, vector<double>& _point_id, int _sequence)
{
	...
	point_3d = _point_3d;                               // 历史回环帧 老帧下的3D点
	point_2d_uv = _point_2d_uv;                         // zlc：这里是 像素点坐标
	point_2d_norm = _point_2d_norm;                     // zlc：这里是相机坐标系下 的 归一化点
    point_3d_depth = _point_3d_depth;                   // △△△△△ zlc新增加的变量，这里的变量值直接取自深度图 △△△△△
	point_id = _point_id;

	has_loop = false;
	loop_index = -1;
	has_fast_point = false;
	loop_info << 0, 0, 0, 0, 0, 0, 0, 0;    // △△△△△ zlc添加：历史帧和当前帧之间的变换关系

	sequence = _sequence;               // △△△△△ zlc添加：着重看明白这个时间序列的操作

    // compute keypoints, brief_desc,
	computeWindowBRIEFPoint();          // △△△△△ zlc添加：计算已有特征点的描述子  △△△△△

    computeBRIEFPoint();                // △△△△△ zlc添加：额外提取fast特征点并计算描述子 △△△△△
...
void KeyFrame::computeBRIEFPoint()
{
	...
	{
        cv::FAST(image, keypoints, fast_th, true);      // △△△△△ zlc新添加的打印语句 △△△△△
        ROS_ERROR("num of FAST: %d",(int)keypoints.size());                     // △△△△△ zlc新添加的打印语句 △△△△△
        keypoints.clear();                                                      // △△△△△ zlc新添加的打印语句 △△△△△
		vector<cv::Point2f> tmp_pts;
		cv::goodFeaturesToTrack(image, tmp_pts, 500, 0.01, 10); // △△△△△ 更改了点数 △△△△△
        ROS_ERROR("num of good features: %d",(int)tmp_pts.size());              // △△△△△ zlc新添加的打印语句 △△△△△
	...
	extractor(image, keypoints, brief_descriptors);     // △△△△△ zlc添加：重中之重，用到的描述子都是在这里提取到的 △△△△△
                                ...
                                const std::vector<cv::KeyPoint>&  keypoints_old_norm)
{
    ...
    for(int i = 0; i < (int)window_brief_descriptors.size(); i ++)
    {
        ...
        // 进行暴力匹配：   用滑窗中的当前帧描述子 和 老的回环帧描述子 进行匹配
                         ...
                         Eigen::Vector3d& PnP_T_old, Eigen::Matrix3d& PnP_R_old)
{
    ...
    else
    {
        ...
        else    // 使用当前帧位姿作为起始位姿，考虑到回环帧距离起始帧并不远
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	*/
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_3d, status);
	    reduceVector(matched_id, status);
        // ROS_WARN("points left after RANSAC: %d", (int)matched_2d_cur.size());       // △△△△△ zlc新添加的 △△△△△
	    #if 1
	    	if (DEBUG_IMAGE)
	        {
	        	int gap = 10;
