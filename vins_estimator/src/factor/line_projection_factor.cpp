...
bool lineProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Vector6d line_w = orth_to_plk(line_orth);       // 从正交表示中转换回普鲁克表示法


    Eigen::Matrix3d Rwb(Qi);     // 直接赋值初始化
    Eigen::Vector3d twb(Pi);       //  直接赋值初始化
    Vector6d line_b = plk_from_pose(line_w, Rwb, twb);      // 转换到body坐标系下的线普鲁克表示
    // std::cout << line_b.norm() <<"\n";
    Eigen::Matrix3d Rbc(qic);
    Eigen::Vector3d tbc(tic);
    Vector6d line_c = plk_from_pose(line_b, Rbc, tbc);      // 线特征在相机坐标系下的普鲁克表示


    // 直线的投影矩阵K为单位阵
    Eigen::Vector3d nc = line_c.head(3);                // △△△△△ 垂线 △△△△△  nc=l  论文中的l 式（33）
    double l_norm = nc(0) * nc(0) + nc(1) * nc(1);
    double l_sqrtnorm = sqrt( l_norm );
    double l_trinorm  = l_norm * l_sqrtnorm;        // 3/2次方


    double e1 = obs_i(0) * nc(0) + obs_i(1) * nc(1) + nc(2);    // 公式（35）
