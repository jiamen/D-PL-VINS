//
// Created by zlc on 2022/4/26.
//

#ifndef _PLVINS_ESTIMATOR_MATH_UTILS_H_
#define _PLVINS_ESTIMATOR_MATH_UTILS_H_
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 3, 4> Qleft_GlobalParameter(const Eigen::QuaternionBase<Derived>& q)
{
    Eigen::Quaternion<typename Derived::Scalar> qq = positify(q);
    qq = qq.conjugate();
...
template <typename Derived>
inline Eigen::Matrix<typename Derived::Scalar, 4, 3> Qleft_LocalParameter(const Eigen::QuaternionBase<Derived>& q)
{
    Eigen::Quaternion<typename Derived::Scalar> qq = positify(q);
...
inline Eigen::Matrix3d g2R(const Eigen::Vector3d& g)
{
    ...
    double yaw = math_utils::R2ypr(R0).x();
...
inline Eigen::Matrix<double, 3, 4> QuaternionGlobalDiff(
        ...
        const Eigen::Vector3d& v)
{
    ...
    JqpJq.leftCols(3) = tmp * Eigen::Matrix3d::Identity() + qxyz * v.transpose() -
                        v * qxyz.transpose() - q.w() * skewSymmetric(v);
...
inline Eigen::Matrix<double, 4, 4> QuaternionInv2Quaternion(const Eigen::Quaterniond& q)
{
...
template <typename T>
Eigen::Matrix<T, 4, 1> TransformPlane(const Eigen::Matrix<T, 4, 1>& PlaneCoeff, const Twist<T>& T_to_from)
{
...
#endif // _PLVINS_ESTIMATOR_MATH_UTILS_H_
