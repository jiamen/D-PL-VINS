...
#include <ceres/jet.h>                  // △△△△△ zlc：结构化信息中新添加的头文件 △△△△△
...
// △zlc：new add  结构化信息中添加的
Eigen::Vector3d pluckerOrigin(const Vector6d &plk);



template <typename T> Eigen::Matrix<T, 3, 3> skew_symmetric(Eigen::Matrix<T, 3, 1> v)
{
...
template <typename T> Eigen::Matrix<T, 6, 1> plk_to_pose(Eigen::Matrix<T, 6, 1> plk_w,
...
template <typename T> Eigen::Matrix<T, 6, 1> plk_from_pose(Eigen::Matrix<T, 6, 1> plk_c,
                                                           Eigen::Matrix<T, 3, 3> Rcw,
                                                           Eigen::Matrix<T, 3, 1> tcw)
{
...
template <typename T> Eigen::Matrix<T, 6, 1> orth_to_plk( Eigen::Matrix<T, 4, 1> orth)
{
