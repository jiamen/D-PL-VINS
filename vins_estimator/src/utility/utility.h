#pragma once
#include <sys/types.h>  // △△△△△ zlc添加的头文件 △△△△△
#include <sys/stat.h>   // △△△△△ zlc添加的头文件 △△△△△
#include <libgen.h>     // △△△△△ zlc添加的头文件 △△△△△
#include <stdio.h>      // △△△△△ zlc添加的头文件 △△△△△
#include <stdlib.h>     // △△△△△ zlc添加的头文件 △△△△△
#include <cmath>
#include <cassert>
#include <cstring>
...
public:
    ...
    // △zlc添加：下面这个函数是结构化信息中添加的，  这个函数似乎没用
    template <typename Derived>
    static Eigen::Matrix<typename Derived::Scalar, 3, 4> Qleft_GlobalParameter(const Eigen::QuaternionBase<Derived>& q)
    {
        Eigen::Quaternion<typename Derived::Scalar> qq = positify(q);
        qq = qq.conjugate();
