...
#include "/home/zlc/Tools/SLAMTools/opencv_contrib-3.4.17/modules/ximgproc/include/opencv2/ximgproc.hpp"
...
{   // 传入的参数是前一帧和当前帧，前一帧的匹配线束，当前帧的匹配线束
    ...
    for (int k = 0; k < good_matches.size(); ++ k)
    {
        ...
        cv::circle(img2, startPoint, 2, cv::Scalar(0, 255, 0), 2);
        cv::circle(img2, endPoint, 2, cv::Scalar(0, 255, 0), 2);
        cv::circle(img2, startPoint2, 2, cv::Scalar(255, 0, 0), 2);
        cv::circle(img2, endPoint2, 2, cv::Scalar(255, 0, 0), 2);
        cv::line(img2, startPoint2, endPoint2, cv::Scalar(r, g, b),2, 8);
        cv::line(img2, startPoint, startPoint2, cv::Scalar(0, 0, 255),1, 8);
        cv::line(img2, endPoint, endPoint2, cv::Scalar(0, 0, 255),1, 8);
    ...
    */
    // imshow("LSD matches1", img1);    // zlc删除：可以显示
    // imshow("LSD matches2", img2);    // zlc删除：可以显示
    // waitKey(1);                             // zlc删除：可以显示
}
...
void LineFeatureTracker::readImage(const cv::Mat& _img)
{
    ...
    //    一种是计算原图像任意像素在映射后图像的坐标位置，另一种是计算变换后图像任意像素反映射在原图像的坐标位置。
    cv::remap(_img, img, undist_map1_, undist_map2_, CV_INTER_LINEAR);
    ...
    // line_detctor.EDline(img, lines, false);      // EDLines
    // std::cout << "EDline:!!!!!!!!!!!!!" << lines.size() << endl;

    std::vector<KeyLine> lsd, keylsd;
    std::vector<KeyLine> myEDKeyLines;
    ...
    // std::cout << "LSD line:!!!!!!!!!!!!!" << lsd.size() << endl;
    ...
    // zlc自己写的：将提取到的EDLine转为opencv的KeyLine数据
    /*
    int class_counter = -1;
    for (int i = 0; i < lines.size(); ++ i)
    {
        ...
        else
        {
            ...
            edline.startPointX = l.line_endpoint[0] * 1;         // octaveScale这里指定为1
            edline.startPointY = l.line_endpoint[1] * 1;         // extremes[1] * octaveScale
            edline.endPointX   = l.line_endpoint[2] * 1;
            edline.endPointY   = l.line_endpoint[3] * 1;
            edline.sPointInOctaveX = l.line_endpoint[0];      // extremes[0]
            edline.sPointInOctaveY = l.line_endpoint[1];
            edline.ePointInOctaveX = l.line_endpoint[2];
            edline.ePointInOctaveY = l.line_endpoint[3];
            edline.lineLength = l.length;
            ...
            // edline.numOfPixels = li.count;
            edline.numOfPixels = l.numOfPixels;

            edline.angle = atan2( ( edline.endPointY - edline.startPointY ), ( edline.endPointX - edline.startPointX ) );
            edline.class_id = ++ class_counter;
            edline.octave = 0;
            edline.size = ( edline.endPointX - edline.startPointX ) * ( edline.endPointY - edline.startPointY );
            edline.response = edline.lineLength / max( img.cols, img.rows );
            edline.pt = Point2f( ( edline.endPointX + edline.startPointX ) / 2, ( edline.endPointY + edline.startPointY ) / 2 );

            myEDKeyLines.push_back(edline);
        }
    }
    */
    ...
    // bd_->compute( img, myEDKeyLines, lbd_descr );
...
//////////////////////////
    ...
    /*
    for ( int i = 0; i < (int) myEDKeyLines.size(); i ++ )
    {
        if( myEDKeyLines[i].octave == 0 && myEDKeyLines[i].lineLength >= 65)
        {
            keylsd.push_back( myEDKeyLines[i] );
            keylbd_descr.push_back( lbd_descr.row( i ) );
        }
    }
    */

    // std::cout << "myEDKeyLines line:!!!!!!!!!!!!!" << myEDKeyLines.size() << endl;
    // std::cout << "myEDKeyLines keylsd:!!!!!!!!!!!!!" << keylsd.size() << endl;
...
//////////////////////////
    ...
    if(curframe_->keylsd.size() > 0)
    {
        ...
        {
            if( lsd_matches[i].distance < 30 )      // 可以定义宏 MATCHES_DIST_THRESHOLD
            {
        ...
        for (size_t i = 0; i < forwframe_->keylsd.size(); ++ i)
        {
            if( forwframe_->lineID[i] == -1)    // 未跟踪到的线
            {
                forwframe_->lineID[i] = allfeature_cnt ++;
        ...
        if( diff_h > 0)    // 补充线条
        {
            ...
            if(diff_h > h_Line_new.size())
                ...
                kkk = int(h_Line_new.size()/diff_h);        // 横向要求线束与差额线束的比
