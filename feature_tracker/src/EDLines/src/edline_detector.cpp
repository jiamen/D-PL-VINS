...
#include "../include/edline_detector.h"
...
EDLineDetector::EDLineDetector()
{
    ...
    ksize_ = 5;
    sigma_ = 1.0;
    gradienThreshold_ = 80;     // 梯度阈值  ***** ORIGINAL WAS 25
...
// 第一步：输入灰度图，利用Edge Drawing算法进行提取，输出一系列连续的像素点
int EDLineDetector::EdgeDrawing(cv::Mat &image, EdgeChains &edgeChains, bool smoothed)
{
    if (!smoothed)
    {
        // 高斯滤波函数，平滑图像噪声     ksize，高斯内核的大小；  sigmaX，表示高斯核函数在X方向的的标准偏差；
    ...
    imageWidth  = image_.cols;      // 列宽
    imageHeight = image_.rows;      // 行高
    unsigned int pixelNum = imageWidth * imageHeight;       // 像素点数量
    ...
    for (unsigned int w = 1; w < imageWidth - 1; w = w + scanIntervals_)
    {
        for (unsigned int h = 1; h < imageHeight - 1; h = h + scanIntervals_)
        {
            ...
            {   // if the direction of pixel is horizontal, then compare with up and down
                ...
                if (pgImg[indexInArray] >= pgImg[indexInArray - imageWidth] + anchorThreshold_ && pgImg[indexInArray] >= pgImg[indexInArray + imageWidth] + anchorThreshold_)
...
EDLineDetector::EDLineDetectorParallel::EDLineDetectorParallel(EDLineDetector *_edline, EdgeChains *_edges,
                                                               double _logNT, cv::Mutex &_lock, std::vector<Line1> *_lines)
   : edline(_edline), edges(_edges), logNT(_logNT), lock(_lock), lines(_lines)
{
        ...
        while (offsetInEdgeArrayE > offsetInEdgeArrayS + minLineLen)    // extract line segments from an edge, may find more than one segments
        {
            ...
            {   // y=ax+b, i.e. ax-y+b=0
                ...
                while (bExtended)
                {
                    ...
                    if (offsetInEdgeArrayS - newOffsetS > 0 && tryTimes < TryTime)
                ...
                if (LineValidation(pEdgeXCors, pEdgeYCors, offsetInEdgeArrayS_init, offsetInEdgeArrayS, lineEqu, direction))
                {
                    ...
                    * yp= (w1^2*y0-w1*w2*x0-w3*w2)/(w1^2+w2^2)  */
                    ...
                    line.numOfPixels = offsetInEdgeArrayS;      // zlc添加
