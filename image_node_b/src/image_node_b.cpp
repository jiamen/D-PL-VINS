              ...
              const sensor_msgs::ImageConstPtr& img_msg) 
{
    ...
    {
        // cv::Point2f startPoint = cv::Point(line_feature_msg->channels[3].values[i], line_feature_msg->channels[4].values[i]);
        cv::Point2f startPoint = cv::Point2f(line_feature_msg->points[i].x, line_feature_msg->points[i].y);
        project(startPoint, K_);
        //cv::Point2f endPoint = cv::Point(line_feature_msg->channels[5].values[i], line_feature_msg->channels[6].values[i]);
        cv::Point2f endPoint = cv::Point2f(line_feature_msg->channels[1].values[i], line_feature_msg->channels[2].values[i]);
        project(endPoint, K_);
        cv::circle(show_img, startPoint, 2, cv::Scalar(255, 0, 0), 3);
        cv::circle(show_img, endPoint, 2, cv::Scalar(255, 0, 0), 3);
...
int main(int argc, char **argv)
{
    ...
    message_filters::Subscriber<sensor_msgs::PointCloud> point_feature_sub(nh, "/plvins_estimator/feature", 1000);
    ...
    // message_filters::Subscriber<sensor_msgs::Image> image_sub(nh, "/d400/color/image_raw", 1000);
